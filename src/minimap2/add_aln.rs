...
// custom error
#[derive(Error, Debug)]
pub enum AddAlnError {
    #[error("sorry that add_aln is not yet fully implemented")]
    NotImplemented,
    #[error("sequence {qname} is missing a cs:Z: tag")]
    MissingCsTag{ qname: String },
    #[error("sequence {qname} failed while parsing CIGAR, RNAME, start or end positions")]
    ParsingError{ qname: String },
    #[error("sequence {qname} does not overlap the scaffold")]
    NoOverlap{ qname: String },
}
...
impl Resolver {
    ...
    pub fn add_aln(
        ...
        Err(AddAlnError::NotImplemented)
        ...
        // let Some(Ok(TagValue::String(cs)))  = aln.data().get(b"cs")  else { 
        //     return Err(AddAlnError::MissingCsTag{qname: Self::get_qname(aln)})
        // };
        // let (
        //     Some(Ok(first_cigar_op)),
        //     Some(Ok(reference_sequence_id)),
        //     Some(Ok(reference_start1)),
        //     Some(Ok(reference_end1))
        // ) = (
        //     aln.cigar().iter().next(),
        //     aln.reference_sequence_id(),
        //     aln.alignment_start(),
        //     aln.alignment_end()
        // ) else {
        //     return Err(AddAlnError::ParsingError{qname: Self::get_qname(aln)})
        // };
        ...
        //     return Err(AddAlnError::NoOverlap{qname: Self::get_qname(aln)})
        // }
